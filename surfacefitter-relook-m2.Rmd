...
```{python}
m1file0 = '/home/james/Documents/MirrorPoints/FI mirrors datafiles/M2_orig_bfit_fori_refs.dat'
m1pts = pd.read_csv(m1file0, skiprows=8, sep=' ', usecols=[1,3,5], names=['x','y','z'], decimal=",")
print(m1pts, type(m1pts.astype('float')))
print(m1pts['x'])
...
```{python}
np.savetxt('tempremakem2.txt', np.array([xre, yre, zre]).T)
```
...
m2pars = (0.991323, 1.114128, 1.092735, 0, 0.102517, 0, 0.258740, 0, -0.107996, -0.136285)

for i in range(len(xre)):
    #print(xre[i], yre[i])    
    z1, z2 = quadraticII(xre[i], yre[i], *m2pars)
...
for i in range(len(xre)):
    #print(xre[i], yre[i])    
    z1s, z2s = quadratic(xre[i], yre[i], *solvedpars)
