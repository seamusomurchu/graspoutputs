...
```{python}
"""remove irregular spacing from measured file"""
```

```{python}
"""find the minumum radius each point has to its next point
then use a histogram to select a threshold radius
for each point create an array"""

def pointradius(px1, py1, px2, py2):
    r = np.sqrt( ( (px2-px1)**2)  + ((py2-py1)**2))
    return r

minvals = np.array(len(data[:,0]))
...
for p in range(len(data[:,0])):
    ...
    for p2 in range(len(data[:,0])):
    
        r = pointradius(data[p,0], data[p,1], data[p2,0], data[p2,1])
        temprads = np.append(temprads,r)
    
    #minimumradius for p
    minrad = min(temprads[np.where(temprads > 0)])
    #print(temprads.shape, minrad)
    temprads = np.array([])
    minrads = np.append(minrads, minrad)
...
print(minrads.shape)
import matplotlib.pyplot as plt
plt.hist(minrads, bins='auto')
```

```{python}
"""so this histogram looks close to what is expected
there is a clear distinction between the sampling spacings
so only points with rmin > 0.006 will be taken
similar algorithim as above"""
...
for p in range(len(data[:,0])):
    ...
    if minrads[p] > 0.006:
        regsampx = np.append(regsampx, data[p,0])
        regsampy = np.append(regsampy, data[p,1])
        regsampz = np.append(regsampz, data[p,2])
    else:
        exx = np.append(exx, data[p,0])
        exy = np.append(exy, data[p,1])
        exz = np.append(exz, data[p,2])
        
print(regsampx.shape)
print(exx.shape)
```

```{python}
"""plot new file and old file"""
# %matplotlib qt
...
#ax.scatter(exx[2::3], exy[2::3], exz[2::3], c='g', marker='.')
...
```{python}
"""nor try to take every second y point from the excluded points array"""
temprads = np.array([])
minrads = np.array([])
for p in range(len(exx)):
    
    for p2 in range(len(exx)):
        
        r = pointradius(0, exy[p], 0, exy[p2])
        temprads = np.append(temprads,r)
...
```{python}
"""sub filtering based on excluded points"""
#included points
regsamp2x = np.array([])
regsamp2y = np.array([])
regsamp2z = np.array([])
...
for p in range(len(exx)):
    ...
    if minrads[p] > 0.0002:
        regsamp2x = np.append(regsamp2x, exx[p])
        regsamp2y = np.append(regsamp2y, exy[p])
        regsamp2z = np.append(regsamp2z, exz[p])
    else:
        exx2 = np.append(exx2, exx[p])
        exy2 = np.append(exy2, exy[p])
        exz2 = np.append(exz2, exz[p])
        
print(regsamp2x.shape)
print(exx2.shape)
```
...
#ax.scatter(data[:,0], data[:,1], data[:,2], c='b', marker='.')
#ax.scatter(regsampx, regsampy, regsampz, c='r', marker='.')
ax.scatter(exx, exy, exz, c='g', marker='.')
ax.scatter(exx2, exy2, exz2, c='k', marker='.')
...
sort array based on y values, then take every second"""
sampdata = np.array((exx, exy, exz)).T
print(sampdata.shape)
print(sampdata[0:5,:])
print(sampdata[0:5,:])
sort = sampdata[np.argsort(sampdata[:,1])]
print(sort[0:5,:])
sort = sort[3::4]
```
...
#ax.scatter(data[:,0], data[:,1], data[:,2], c='b', marker='.')
#ax.scatter(regsampx, regsampy, regsampz, c='r', marker='.')
#ax.scatter(exx, exy, exz, c='g', marker='.', alpha=0.5)
...
```{python}
"""looks okay now append with sort 
sort = sampdata[np.argsort(sampdata[:,1])]
print(sort[0:5,:])
sort = sort[3::4]"""

newx = np.append(regsampx, sort[:,0])
newy = np.append(regsampy, sort[:,1])
newz = np.append(regsampz, sort[:,2])

# newdata = np.array((newx, newy, newz)).T
# print(newdata.shape)

#write file

m.surfacewriter("resampled","measured", newx/1000, newy/1000, newz/1000)
```
